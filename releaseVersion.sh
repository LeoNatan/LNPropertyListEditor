...
CURRENT_VERSION=$(/usr/libexec/PlistBuddy LNPropertyListEditor/LNPropertyListEditor/Info.plist -c "Print CFBundleShortVersionString")
...
/usr/libexec/PlistBuddy LNPropertyListEditor/LNPropertyListEditor/Info.plist -c "Set CFBundleShortVersionString $NEXT_VERSION" -c "Set CFBundleVersion 1"
...
gh release create --repo LeoNatan/LNPropertyListEditor "$NEXT_VERSION" --title "v$NEXT_VERSION" --notes-file "${RELEASE_NOTES_FILE}"
